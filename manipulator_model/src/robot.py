...
class Robot:
    ...
    def jacobianComputation(self):
        ...
        l2s1s2 = self.l2*np.sin(self.theta[0])*np.sin(self.theta[1])
        l2s1c2 = self.l2*np.sin(self.theta[0])*np.cos(self.theta[1])
        l2c1s2 = self.l2*np.cos(self.theta[0])*np.sin(self.theta[1])
        l2c1c2 = self.l2*np.cos(self.theta[0])*np.cos(self.theta[1])

        d4s234 = self.d4*np.sin(self.theta[1]+self.theta[2]+self.theta[3])
        l3s23 = self.l3*np.sin(self.theta[1]+self.theta[2])
        l2s2 = self.l2*np.sin(self.theta[1])
